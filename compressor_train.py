...
_SCRIPT_DIR = Path(__file__).resolve().parent
_DEFAULT_TRAIN_DATA = _SCRIPT_DIR / "data" / "main_data.xlsx"
_DEFAULT_MODEL_OUT = _SCRIPT_DIR / "data" / "quantile_models.pkl"
...
SPEC_COLUMNS = [
    "rated_fad_cfm",
    "rated_motor_output_kw",
    "tolerance_flow_pct",
    "flow_tolerance_pct(%)",
    "(motor_power_tolerance_pct(%))",
    "delivery_pressure_kg_cm2g",
    "spc_kw_per_m3_min",
    "tolerance_spc_pct",
]
...
def engineer_features_batch(run_df, stable_ref, stable_std):
    ...
    for i in range(20, len(run_df)):
        ...
        for col in SPEC_COLUMNS:
            feats[col] = run_df[col].iloc[i]

                # Relative features
        if run_df["rated_fad_cfm"].iloc[i] != 0:
            feats["fad_ratio"] = (
                w["fad_cfm"].iloc[i] /
                run_df["rated_fad_cfm"].iloc[i]
            )
        else:
            feats["fad_ratio"] = 0

        if run_df["rated_motor_output_kw"].iloc[i] != 0:
            feats["motor_power_ratio"] = (
                w["motor_output_power_kw"].iloc[i] /
                run_df["rated_motor_output_kw"].iloc[i]
            )
        else:
            feats["motor_power_ratio"] = 0

        if run_df["rated_fad_cfm"].iloc[i] != 0:
            feats["fad_deviation_pct"] = (
                (w["fad_cfm"].iloc[i] - run_df["rated_fad_cfm"].iloc[i]) /
                run_df["rated_fad_cfm"].iloc[i]
            ) * 100
        else:
            feats["fad_deviation_pct"] = 0

        if run_df["rated_motor_output_kw"].iloc[i] != 0:
            feats["motor_power_deviation_pct"] = (
                (w["motor_output_power_kw"].iloc[i] - run_df["rated_motor_output_kw"].iloc[i]) /
                run_df["rated_motor_output_kw"].iloc[i]
            ) * 100
        else:
            feats["motor_power_deviation_pct"] = 0
...
def augment_runs(df, stable_ref, stable_std, stable_time, n_runs=N_AUG_RUNS):
    ...
    for run_id in range(n_runs):
        ...
        for s in SENSORS:
            aug[s] = aug[s] + np.random.normal(
                0,
                stable_std[s] * noise_scale,
                len(aug)
            )
        ...
        for i in range(20, len(aug)):
            ...
            sensor_future = [
                float(aug["airend_discharge_temp_c"].iloc[future_idx]),
                float(aug["fad_cfm"].iloc[future_idx]),
                float(aug["motor_output_power_kw"].iloc[future_idx]),
            ]
        ...
        log(
            ...
            f"Speed={speed:.2f} | Noise={noise_scale:.2f} | "
            f"Samples={len(rows)}"
        )
...
def make_objective(Xs, y, groups):
    ...
    def objective(trial):
        ...
        for tr, vl in gkf.split(Xs, y, groups):
            ...
            maes.append(
                mean_absolute_error(y[vl][:, 0], preds[:, 0])
            )
...
def train(data_path, output_path="model.pkl"):
    ...
    # Clean column names to avoid hidden spacing issues
    ...
    stable_ref = phase3[SENSORS].mean().to_dict()
    stable_std = phase3[SENSORS].std().replace(0, 1e-6).to_dict()
    stable_time = float(phase3["elapsed_time_min"].min())
    ...
    bundle = {
        ...
        "target_names": [
            "time_to_stability",
            "future_airend_discharge_temp_c",
            "future_fad_cfm",
            "future_motor_output_power_kw",
        ]
    }
